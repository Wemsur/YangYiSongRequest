...
- 2026-09-05 播出单卡片堆叠改成连续跟手：位置按带小数的索引插值，当前卡片跟手划走并缩小后退，旁边那张同时往中间走、放大前来，越过半格交换 z；抬手只补齐小数，剩下由 CSS 过渡收尾，实测抬手前后的 transform 完全一致，没有跳变。容器高度也在两张之间插值。
  - 一格以内不再淡出。第一版让相邻卡片半透明，交接瞬间两层文字互相透、糊成一片；卡片本来就是不透明的纸，靠遮挡和缩放表达前后即可，只有两格开外才淡出隐藏。
  - 拖动过程中所有卡片都裁到容器高度。只留「最前那张」不裁会漏：拖到一半时最前的那张可能比插值出来的容器高，内容会溢出压到搜索区上。
  - 顺带把 SFC 分块写入时留下的多余 `</script>` 清掉——这个坑连着踩了两次，以后分块写 Vue 组件不要在占位文件里留闭合标签。

- 2026-09-05 首页播出单从按天纵向排列改成卡片堆叠轮换（`web/src/components/PlaylistDeck.vue`）：默认今天居中，左昨天右未来，手机滑动、桌面按钮 + 拖动、键盘方向键都能翻，上方日期标签兼作导航。
  - 卡片绝对定位所以不占布局高度，容器高度用 ResizeObserver 跟着当前卡片走；后面那几张必须裁到同高，否则更长的卡片会溢出压住下面的搜索区（第一版就踩了这个）。
  - 拖动阈值 48px，位移超过 8px 才算拖拽并吞掉随后那次 click，避免误触卡片里的试听按钮；到头加阻尼。
  - 实测：桌面翻页按钮、日期标签、模拟滑动、方向键四条路径都能切换，375px 无横向滚动，卡片内试听按钮照常可点。
